...
class CoreAIv2Engine:
    ...
    def load_from_dict(cls, data: dict) -> "CoreAIv2Engine":
        ...
        if nm:
            engine.nm_engine.uni   = Counter(nm["uni"])
            engine.nm_engine.bi    = defaultdict(Counter, {k:Counter(v) for k,v in nm["bi"].items()})
            engine.nm_engine.tri   = defaultdict(Counter, {k:Counter(v) for k,v in nm["tri"].items()})
